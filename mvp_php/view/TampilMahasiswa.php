...
class TampilMahasiswa implements KontrakView
{
	...
	function tampil()
	{
		...
		for ($i = 0; $i < $this->prosesmahasiswa->getSize(); $i++) {
            ...
            <td>
                <a href='edit.php?nim=" . $this->prosesmahasiswa->getNim($i) . "' class='btn btn-warning btn-sm'>Edit</a>
                <a href='delete.php?nim=" . $this->prosesmahasiswa->getNim($i) . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data?\")'>Hapus</a>
            </td>
			</tr>";
		}
